...
func main() {
        ...
        seedRedemptions(db, tenantID, counterIDs, userIDs)
        seedGateLogs(db, tenantID, gateIDs, userIDs)
        seedAuditLogs(db, tenantID, userIDs)
...
func seedUsers(db *gorm.DB, tenantID string) map[string]string {
        ...
        userDefs := []struct {
                ...
                {"PARTICIPANT", "Rina Kusuma", "fan6@gmail.com", "mock-participant-006"},
                {"PARTICIPANT", "Bagas Saputra", "fan7@gmail.com", "mock-participant-007"},
                {"PARTICIPANT", "Larasati Dewi", "fan8@gmail.com", "mock-participant-008"},
                {"PARTICIPANT", "Hendra Wijaya", "fan9@gmail.com", "mock-participant-009"},
                {"PARTICIPANT", "Putri Amalina", "fan10@gmail.com", "mock-participant-010"},
        }
...
func seedDemoOrders(db *gorm.DB, tenantID, eventID string, userIDs map[string]string, ttIDs map[string]string) {
        ...
        fan6 := userIDs["PARTICIPANT_fan6@gmail.com"]
        fan7 := userIDs["PARTICIPANT_fan7@gmail.com"]
        fan8 := userIDs["PARTICIPANT_fan8@gmail.com"]
        fan9 := userIDs["PARTICIPANT_fan9@gmail.com"]
        fan10 := userIDs["PARTICIPANT_fan10@gmail.com"]
        ...
        cat2TT := ttIDs["CAT 2"]
        cat3TT := ttIDs["CAT 3"]
        _ = cat3TT // not yet used in demo orders
        ...
        // ── Order 6: fan6 buys 1x VIP (PAID) ────────────────────────────
        order6 := createDemoOrder(db, tenantID, fan6, eventID, "paid", 2500000,
                []orderItemDef{{ttID: vipTT, qty: 1, price: 2500000}}, &now)
        createDemoTicket(db, tenantID, order6.ID, vipTT, eventID, fan6,
                "Rina Kusuma", "fan6@gmail.com", "VIP-B-5-10", eventTitle, "VIP", "active", nil, "")

        // ── Order 7: fan7 buys 1x Festival (PAID) ──────────────────────
        order7 := createDemoOrder(db, tenantID, fan7, eventID, "paid", 500000,
                []orderItemDef{{ttID: festivalTT, qty: 1, price: 500000}}, &now)
        createDemoTicket(db, tenantID, order7.ID, festivalTT, eventID, fan7,
                "Bagas Saputra", "fan7@gmail.com", "Festival", eventTitle, "Festival", "active", nil, "")

        // ── Order 8: fan8 buys 1x CAT 1 + 1x VIP (PAID, multi-item) ──
        order8 := createDemoOrder(db, tenantID, fan8, eventID, "paid", 4000000,
                []orderItemDef{{ttID: cat1TT, qty: 1, price: 1500000}, {ttID: vipTT, qty: 1, price: 2500000}}, &now)
        createDemoTicket(db, tenantID, order8.ID, cat1TT, eventID, fan8,
                "Larasati Dewi", "fan8@gmail.com", "CAT1-C-6-12", eventTitle, "CAT 1", "active", nil, "")
        createDemoTicket(db, tenantID, order8.ID, vipTT, eventID, fan8,
                "Larasati Dewi", "fan8@gmail.com", "VIP-B-7-18", eventTitle, "VIP", "active", nil, "")

        // ── Order 9: fan9 buys 1x CAT 2 (PENDING — no ticket yet) ────
        createDemoOrder(db, tenantID, fan9, eventID, "pending", 1000000,
                []orderItemDef{{ttID: cat2TT, qty: 1, price: 1000000}}, nil)

        // ── Order 10: fan10 buys 1x Festival (CANCELLED — no ticket) ──
        createDemoOrder(db, tenantID, fan10, eventID, "cancelled", 500000,
                []orderItemDef{{ttID: festivalTT, qty: 1, price: 500000}}, nil)

        // ── Update sold counts ─────────────────────────────────────────────
        db.Model(&models.TicketType{}).Where("id = ?", vvipTT).Update("sold", 1)
        db.Model(&models.TicketType{}).Where("id = ?", vipTT).Update("sold", 4)
        db.Model(&models.TicketType{}).Where("id = ?", cat1TT).Update("sold", 2)
        db.Model(&models.TicketType{}).Where("id = ?", cat2TT).Update("sold", 2)
        db.Model(&models.TicketType{}).Where("id = ?", festivalTT).Update("sold", 2)

        log.Println("✅ Demo Orders & Tickets: 10 orders, 10 tickets created")
...
func seedRedemptions(db *gorm.DB, tenantID string, counterIDs []string, userIDs map[string]string) {
        var count int64
        db.Model(&models.Redemption{}).Count(&count)
        if count > 0 {
                log.Println("⏭️  Redemptions exist")
                return
        }

        // Find tickets that have been redeemed or are inside the venue
        var tickets []models.Ticket
        db.Where("status IN ?", []string{"redeemed", "inside"}).Find(&tickets)
        if len(tickets) == 0 {
                log.Println("⏭️  No redeemed/inside tickets found for redemptions")
                return
        }

        counterStaffID := userIDs["COUNTER_STAFF_counter1@seleevent.id"]

        wristbandMap := map[string]struct {
                color string
                wType string
        }{
                "VVIP":      {"Gold", "VVIP"},
                ...
                "CAT 2":     {"Green", "CAT 2"},
                "CAT 3":     {"Orange", "CAT 3"},
                "Festival":  {"Red", "Festival"},
        }

        var created int
        for _, t := range tickets {
                wb, ok := wristbandMap[t.TicketTypeName]
                if !ok {
                        continue
                }
                wristbandCode := ""
                if t.WristbandCode != nil {
                        wristbandCode = *t.WristbandCode
                } else {
                        wristbandCode = "WB-" + t.TicketTypeName + "-" + t.TicketCode[:8]
                }

                notes := "Demo wristband redemption"
                r := models.Redemption{
                        TenantID:       tenantID,
                        TicketID:       t.ID,
                        CounterID:      counterIDs[0],
                        StaffID:        counterStaffID,
                        WristbandCode:  wristbandCode,
                        WristbandColor: wb.color,
                        WristbandType:  wb.wType,
                        Notes:          &notes,
                }
                mustCreate(db, &r, "redemption for ticket "+t.TicketCode)
                created++
        }
        log.Printf("✅ Redemptions: %d created", created)
}

// ─── GATE LOGS ────────────────────────────────────────────────────────────

func seedGateLogs(db *gorm.DB, tenantID string, gateIDs []string, userIDs map[string]string) {
        var count int64
        db.Model(&models.GateLog{}).Count(&count)
        if count > 0 {
                log.Println("⏭️  GateLogs exist")
                return
        }

        // Find tickets with "inside" status (have entered the venue)
        var tickets []models.Ticket
        db.Where("status = ?", "inside").Find(&tickets)
        if len(tickets) == 0 {
                log.Println("⏭️  No inside tickets found for gate logs")
                return
        }

        gateStaffID := userIDs["GATE_STAFF_gate1@seleevent.id"]

        var created int
        for _, t := range tickets {
                entryNotes := "Demo entry scan"
                gl := models.GateLog{
                        TenantID: tenantID,
                        TicketID: t.ID,
                        GateID:   gateIDs[2], // Gate 3 - General
                        StaffID:  &gateStaffID,
                        EventID:  t.EventID,
                        Action:   "entry",
                        Notes:    &entryNotes,
                }
                mustCreate(db, &gl, "gate log entry for ticket "+t.TicketCode)
                created++
        }
        log.Printf("✅ GateLogs: %d created", created)
}

// ─── AUDIT LOGS ───────────────────────────────────────────────────────────

func seedAuditLogs(db *gorm.DB, tenantID string, userIDs map[string]string) {
        var count int64
        db.Model(&models.AuditLog{}).Count(&count)
        if count > 0 {
                log.Println("⏭️  AuditLogs exist")
                return
        }

        fan1ID := userIDs["PARTICIPANT_fan1@gmail.com"]
        organizerID := userIDs["ORGANIZER_organizer@sheilaon7.id"]
        counter1ID := userIDs["COUNTER_STAFF_counter1@seleevent.id"]
        ...
        auditDefs := []struct {
                ...
                {fan1ID, "user.login", "auth", "User logged in via Google OAuth", "182.1.10.45"},
                {organizerID, "order.create", "orders", "Created order for VVIP ticket — Sheila On 7", "182.1.10.45"},
                {counter1ID, "ticket.redeem", "redemptions", "Redeemed ticket for wristband WB-SLVR1", "10.0.0.5"},
        }

        for _, def := range auditDefs {
                al := models.AuditLog{
                        TenantID: tenantID,
                        UserID:   def.userID,
                        Action:   def.action,
                ...
                mustCreate(db, &al, "audit log "+def.action)
        }
        log.Printf("✅ AuditLogs: %d created", len(auditDefs))
}
